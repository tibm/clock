...
**Status:** v0.16 draft · **Owner:** you (FW/HW) · **Last updated:** 2026-07-12
...
**Design notes:**
- **Homing (optical, single sensor, no magnets):** one **reflective optical sensor** (Everlight **ITR8307**, 3.4×1.5×1.1 mm) sits behind a small **hole punched in the dial** at a chosen index radius that *both* hands sweep over. Because the MCU drives the two shafts independently, home them **sequentially** so the minute-over-hour overlap never matters:
  ...
  - *Sensor locked = ITR8307 (smallest hole):* it's the **smallest** reflective part evaluated (**3.4×1.5×1.1 mm**) → the **smallest dial hole**. Its short range is fine because the hands sweep **<3 mm** off the dial (index mark on the underside). The longer-range **Vishay TCND5000** (6×4.3×3.75 mm, peak 6 mm, range 2–25 mm) was considered for a >3 mm stand-off and **dropped — ~5× the footprint = a bigger hole**; keep the hands close instead.
...
- **Driver:** **2× TB6612FNG** (SSOP-24, hand-solderable, no exposed pad) — one per shaft; VM 5 V, VCC 3.3 V; **PWM the IN pins with PWMA/PWMB tied high → sign-magnitude microstep in 8 GPIO** (parity with the old DRV8835). *DRV8835/DRV8833 are WSON/HTSSOP — replaced for hand-assembly.* **Homing:** **1× reflective optical sensor** (Everlight ITR8307) behind a hole in the dial — single-sensor sequential homing, **no hand magnets** (replaces the earlier 2× DRV5032 Hall + magnets; see homing note above). **Libs:** SwitecX25 / GewoonGijs-VID28 / AccelStepper.
...
| Hand homing ⭐ | **Everlight ITR8307** (reflective optical) | 1 sensor, sequential homing, **no hand magnets** (§5); **smallest reflective pkg (3.4×1.5×1.1 mm) → smallest dial hole** | ~$0.5 |
...
| Hand homing (×1, optical) | Everlight ITR8307 (or QRE1113 reflective breakout) | ~$1 | [DigiKey 2693862 ✅](https://www.digikey.com/en/products/detail/everlight-electronics-co-ltd/ITR8307-TR8/2693862) |
...
| ⭐ | IO expander (I²C, 16-bit) | **Microchip MCP23017** (SPI twin MCP23S17) — offloads SPK_SD / 12 V-EN / stepper STBY / buttons / enc-SW / PD-PG / CHRG / FAULT; INT→1 GPIO (net −7) | **SOIC-28 W / SSOP-28** | ~$1.3 | [search](https://www.digikey.com/en/products/result?keywords=MCP23017) |
...
| ⭐ | Hand homing ×1 (optical, reflective — **no magnets**) | Everlight ITR8307 (single sensor, sequential homing; **smallest reflective pkg → smallest hole**) | **3.4×1.5×1.1 mm**, 4-SMD | ~$0.5 | [2693862](https://www.digikey.com/en/products/detail/everlight-electronics-co-ltd/ITR8307-TR8/2693862) |
| ⭐ | Audio amp | TAS5760MDAPR | **HTSSOP-32** | ~$4–6 | [search](https://www.digikey.com/en/products/result?keywords=TAS5760MDAPR) |
...
**Core electronics subtotal (excl. speaker/cell/PCB): ~$130–155** (power electronics ≈ +$16–18 — the LT3652 is the priciest swap; the 2a sensor chain is 3 Adafruit **breakout modules** — BME688 + TSL2591 + LIS3DH ≈ **$31** (≈ +$18–20 vs bare parts; 2b daughterboard recovers most of that)). With speaker + user-supplied 18650 + holder + 4-layer PCB + passives ≈ **~$200–240**. +CO₂/PM ≈ +$62. *(Cell is user-supplied; safety HW is non-negotiable — see [`power.md`](power.md).)*
...
- **Hand homing:** **1× reflective optical** (Everlight **ITR8307** ⭐, **3.4×1.5×1.1 mm**) behind a punched dial hole — single-sensor sequential, **no magnets**, smallest part → smallest hole; Vishay **TCND5000** (6×4.3×3.75 mm) evaluated for a long stand-off and dropped; ToF (VL53L1x) rejected (coarse angle). *(Replaces 2× DRV5032 Hall.)*
