...
- Behind the glass: **walnut dial background** (same wood as the walls), **aluminum hour + minute hands**, **no numerals — 12 black hour markers**. A small punched hole at the index radius serves the QRE1113 homing sensor (§5).
  - ⚠️ **Doc ↔ CAD discrepancy (2026-08-10):** this line and `CLAUDE.md` both said "12 **dots**", but [`cad/clock_plate.svg`](cad/clock_plate.svg) draws 12 **rectangular batons**, 1.70 × 5.71 mm, spanning **r 35.14 → 40.85 mm**. The CAD is what will be cut, so it wins; `ux/` renders batons. Two further gaps to close in the CAD: the **Ø90 mm opening and the five status-LED holes are not in the SVG** (it has the plate outline, the corner screws, the hub and the homing window only), and the **40 mm minute hand reaches into the baton band** — intended, or should the batons move out?
...
|---|---|---|---|---|
| 1 | `bell` — **alarm on/off** | **clockwise = ON, anticlockwise = OFF** (direction, not distance) | ON → **slow red breath** · OFF → **slow white breath** | ON → the **alarm time** · OFF → **both hands on the 6** (a reading no clock can show) |
| 2 | `alarm-clock` — **set the alarm** | move the alarm time — **one detent, one minute**, and a spin is worth the minutes you spun it, paid out at the speed the hands can draw | *the same as `bell`*: it answers the same question | the alarm time being set, live |
| 3 | `clock` — **set the time** | move the wall clock, same feel | **steady white** | the time being set, live — opening on the clock's own time, or 12:00 if it has never been set |
| 4 | `volume-1` — **volume** | up/down, a gentle chime repeating at that level | **steady white** | **a gauge:** 12:00 = 0 %, 10:00 = 100 % (300° of dial, both hands together, **swept** — the 10-to-12 sector is off the scale and the hands never enter it) |
...
- **Timeout:** 5 s without input → exit to normal, keeping whatever was set. **One timeout, every mode, pairing included** — a control with no labels can afford exactly one rule about how long it waits for you.
- **Long press** (~0.8 s) → commit and leave from any mode.
- **Hold 10 s → BLE pairing:** all five LEDs breathe **blue in sync**. It commits at the ten-second mark, while the knob is still down, so the gesture shows itself rather than hiding until you let go. A press leaves; a finger on the knob counts as input, so it stays for as long as it is held and then obeys the same 5 s as everything else.
...
- **Turning is a direction, and the hands obey it.** Winding a time forward moves both hands forward, however far you wind: past the half hour the minute hand's next place is more than half a turn away, and "go the shorter way" would take it backwards under a steady clockwise turn (`FIRMWARE.md` §6.6e).
- **And the dial can always keep up.** The movement draws about twenty minutes of dial a second, so that is as fast as a knob may move the *setting*: counts are banked and paid out at that pace rather than multiplied by a fast-turn curve. A number that outruns its own readout is not a setting, it is a guess — the minute hand's target wraps, a hand in flight gets re-aimed behind itself, and one steady turn produces a hand that stutters or backs up (`FIRMWARE.md` §6.6d, §16c).
...
| ⭐ | Sensor-board connector (J7, off-board I²C + power: GND/3V3/SDA/SCL/INT + **ALS_INT**) | **JST B6B-ZR(LF)(SN)** header ×2 (one per board, TH vertical, shrouded/keyed, 1 A) + **A06ZR06ZR28H152B** pre-crimped cable ×1 (ZH, 6-pos, socket↔socket, 28 AWG, **152 mm** — was 102 mm until 2026-08-06; the enclosure needs ≥120 mm) — same ZH family as the knob J10, whose harness is a **pigtail** (§16d). ⚠ **2.7 mm tail on a 1.6 mm board:** JST specs `B6B-ZR-3.4` for 1.6 mm, but that variant is made-to-order only (MOQ 2,000, 16 wk) — deviation accepted 2026-08-07, see `kicad/PCB_NOTES.md`. ⚠ **J7 and J10 are the same connector 13.5 mm apart** and J10 carries **+5 V on pin 2**: swapping the two harnesses destroys all three sensors. A 1×07 J7 (which would make that physically impossible) was considered and **rejected 2026-08-08** — `kicad-sensor/REVIEW.md` — so **the labels are the keying: label both harnesses** | **TH, 6-pos, 1.5 mm** | ~$0.24 ×2 + ~$1.54 | [DigiKey ✅](https://www.digikey.com/en/products/detail/jst-sales-america-inc/B6B-ZR(LF)(SN)/455-1661-ND/926568) [DigiKey ✅](https://www.digikey.com/en/products/result?keywords=A06ZR06ZR28H102B) |
...
| ⭐ | Cell protector (independent) | **HYCON HY2111-HB** (OV 4.28 V / OD 2.90 V, **discharge-OC V_DIP 200 mV**) + AOSD32334C dual-N FET — **-HB not -GB**: the -GB's 150 mV trips at ~1.9 A worst case, which a loud plugged alarm exceeds (§16, `kicad/REVIEW.md` #6). Every other threshold is identical; check the marking on arrival | **SOT-23-6 + SO-8** | ~$0.1 + ~$0.8 | [LCSC ✅](https://www.lcsc.com/product-detail/Battery-Management-ICs_HYCON-Tech-HY2111-GB_C82747.html) [DigiKey ✅](https://www.digikey.com/en/products/detail/alpha-omega-semiconductor-inc/AOSD32334C/11567511) |
...
| ⭐ | 3.3 V rail (from 5 V) | **TI TLV62569DBVR** buck (2 A, 2.5–5.5 V in) | **SOT-23-5** *(DBV = SOT23-5 per datasheet §5; DDC is the 6-pin — corrected 2026-07-27)* | ~$0.3 | [DigiKey ✅](https://www.digikey.com/en/products/detail/texas-instruments/TLV62569DBVR/7688370) |
...
| ⭐ | Speaker driver | Dayton DMA58-4 (2″ FR) — **4 Ω, not the 8 Ω DMA58-8** (the −4.1 dBFS limiter ceiling, `A_GAIN` 19.2 dBV and the L5/L6 10 µH/Isat-3.0 A sizing all assume 4 Ω; 8 Ω also wants 22 µH at L5/L6). **Buys +3 dB on battery only** — plugged, both impedances land on ~8 W because L5/L6 saturation, not the rail, is the ceiling (§16 note). **Ships with bare solder tabs — no wire, no connector**; J3 harness in §16d | 56×56×32 mm | ~$19 | [PartsExpress ✅](https://www.parts-express.com/Dayton-Audio-DMA58-4-2-Dual-Magnet-Aluminum-Cone-Full-Range-Driver-4-Ohm-295-582?quantity=1) |
...
### 16c. Discrete semiconductors + magnetics (switchers, protection, filter)

> Values in [`power_values.md`](power_values.md); enumerated here so the production BOM is complete (these were on the schematic/PCB but not previously listed). Inductor MPNs verified 2026-07-24 against the [Coilcraft XAL40xx datasheet](datasheet/inductor_coilcraft_xal40xx.pdf) (Isat = 30 % L-drop). ⚠️ = action needed before fab.

| Status | Block (refs) | MPN | Size / pkg | ~Unit | Source |
|:--:|-------|-----|-----------|-------|--------|
| ⭐ | LT3652 buck inductor (L1) | **Coilcraft XAL4040-103MEC** — 10 µH, Isat 3.0 A, Irms 2.2 A, DCR 84 mΩ (I_CHG 1 A, big margin) | 4.0×4.0×4.0 mm | ~$1.0 | [DigiKey](https://www.digikey.com/en/products/result?keywords=XAL4040-103MEC) |
| ⭐ | 5 V-boost inductor (L2) | **Coilcraft XAL4020-102MEC** — 1 µH, Isat 8.7 A, Irms 6.7 A | 4.0×4.0×2.1 mm | ~$0.9 | [DigiKey](https://www.digikey.com/en/products/result?keywords=XAL4020-102MEC) |
| ⭐ | 3.3 V-buck inductor (L3) | **Coilcraft XAL4020-222MEC** — 2.2 µH, Isat 5.6 A, Irms 4.0 A | 4.0×4.0×2.1 mm | ~$0.9 | [DigiKey](https://www.digikey.com/en/products/result?keywords=XAL4020-222MEC) |
| ⭐ | 12 V-boost inductor (L4) | **Coilcraft XGL5050-472MEC** — 4.7 µH, **Isat 9.7 A**, DCR 16 mΩ, in stock; datasheet [`inductor_coilcraft_xgl5050.pdf`](datasheet/inductor_coilcraft_xgl5050.pdf). **Implemented 2026-07-24** on the 5×5 mm `L_Coilcraft_XAL5050-XXX` land (replaces the 4×4 mm `XAL4030-472`, whose 4.6 A Isat was below the ~6 A the 1S→12 V boost peaks at). **Land verified 2026-07-27:** the KiCad `XAL5050-XXX` pads (1.18 × 4.7 mm on a 3.31 mm pitch) are *identical* to the XGL5050 recommended land pattern (doc 1577-1), and the XGL body max 5.48 × 5.68 mm equals the XAL50xx envelope the footprint is drawn for → true drop-in, no edit needed | 5.3×5.5×5.0 mm | ~$1.3 | [DigiKey](https://www.digikey.com/en/products/result?keywords=XGL5050-472MEC) |
| ⭐ | Speaker LC-filter inductor ×2 (L5, L6) | **Coilcraft XAL4040-103MEC** — 10 µH, **Isat 3.0 A**, Irms 2.2 A (20 °C rise) / 3.1 A (40 °C rise), DCR 84 mΩ. They carry the full speaker current and the amp's OCP (7 A/BTL → **~14 A in PBTL**) is 4–5× past their Isat, so **firmware is the only guard**: the DSP limiter is capped at **8 W into 4 Ω** (peak 2.0 A + 0.38 A ripple = 79 % of Isat) — see [`FIRMWARE.md`](FIRMWARE.md) §6.2. ⚠️ still bench-confirm with a current probe on L5 at max volume | 4.0×4.0×4.0 mm | ~$1.0 ×2 | [DigiKey](https://www.digikey.com/en/products/result?keywords=XAL4040-103MEC) |
| ⭐ | Level-shift / rail-disconnect N-FET ×3 (Q1 `FULLCHG_EN`, Q3 `VBAT_DIV_EN`, Q8 `CELL_TEST`) | **2N7002** (60 V, 0.3 A N-MOSFET) — datasheet [`fet_2n7002.pdf`](datasheet/fet_2n7002.pdf) | SOT-23 | ~$0.10 | [DigiKey](https://www.digikey.com/en/products/result?keywords=2N7002) |
| ⭐ | LT3652 BOOST refresh diode (D10) | **BAT46W-E3-08** (Schottky, 100 V) — schematic value `BAT46`; the SOD-123 part is **BAT46W**. Datasheet [`diode_schottky_bat46w.pdf`](datasheet/diode_schottky_bat46w.pdf) | SOD-123 | ~$0.30 | [DigiKey](https://www.digikey.com/en/products/result?keywords=BAT46W-E3-08) |
| ⭐ | Charger catch + 12 V-boost rectifier (D11, D20) | **B340A-13-F** (Schottky, 40 V / 3 A) — datasheet [`diode_schottky_b340a.pdf`](datasheet/diode_schottky_b340a.pdf) | SMA / DO-214AC | ~$0.30 | [DigiKey](https://www.digikey.com/en/products/result?keywords=B340A-13-F) |
| ⭐ | Amp-mux 12 V blocking diode (D30) | **B340A-13-F** (SMA). **Implemented 2026-07-24** — was `SS34` (an SMC part) on an SMA land; now B340A to match the land + consolidate with D11/D20 (one Schottky P/N + [datasheet](datasheet/diode_schottky_b340a.pdf)). The mux leg only needs ≥1.5 A / 40 V, so 3 A is ample. | SMA / DO-214AC | ~$0.30 | [DigiKey](https://www.digikey.com/en/products/result?keywords=B340A-13-F) |
...
**Speaker impedance — buy the 4 Ω `DMA58-4` (Parts Express 295-582), not the 8 Ω DMA58-8.**
The whole audio chain is dimensioned for 4 Ω: TAS5760M PBTL off the 12 V boost, the firmware
limiter ceiling (−4.1 dBFS = 8 W into 4 Ω), the `A_GAIN` 19.2 dBV setting, and the L5/L6
XAL4040 Isat-3.0 A sizing all derive from it. 8 Ω additionally wants **22 µH** at L5/L6 per TI's
filter table, so it is two more BOM lines, not just a driver swap.

*(Corrected 2026-08-08 — this note used to claim "8 Ω would halve output to ~4 W". **It would
not, when plugged.** At 4 Ω the plugged ceiling is not the rail, it is **L5/L6 saturation**: the
rail would allow 16.5 W but the limiter caps it at 8 W to keep peak inductor current at 79 % of
Isat. At 8 Ω you are rail-limited at 11.5 V → 8.26 W, at only 61 % of Isat. **Plugged, the two
impedances give the same ~8 W and the same SPL.** The difference is real only on **battery**,
where PVDD muxes to ~5 V and the rail clips: **4 Ω = 3.1 W, 8 Ω = 1.6 W — a genuine +3 dB for
the 4 Ω.** So the case for 4 Ω is battery loudness and matching the constants already chosen,
not plugged output.)*
...
|------|----------|-----------|
| 2026-08-16 | **The knob may not outrun the dial (§12 above; `FIRMWARE.md` §6.6d, §16c).** The fast-turn acceleration curve is gone from `alarm`/`clock` — one detent is one minute — and counts are **banked and paid out at the speed the hands can draw** (~20 minutes of dial a second, derived from `motion tune v_max`). The volume gauge keeps its curve: 300° end to end, it cannot wrap | Two reports, one cause: a fast clockwise drag made one 20 ms poll worth two hours, which is exactly two turns of the minute hand — so it stood still while the hour hand moved, and looked like it was *following* it. Anticlockwise, the setting ran more than a turn ahead, the minute hand's target wrapped, and a hand in flight was re-aimed at somewhere it had already passed, so it backed up. A readout that cannot keep up with the number it is showing is not a readout; the honest limit is the movement's own speed. Verified with a real mouse drag in `ux/`: two knob turns each way, 12:1 between the hands, not one sample against the turn |
| 2026-08-15 | **UX second pass, after living with it (§12 above; `FIRMWARE.md` §16b).** (1) **The knob's direction now reaches the hands** — a wind past the half hour moved the minute hand more than half a turn, `motion` took the shorter way, and it ran *backwards* under a steady clockwise turn; targets carry a direction and are chased in the hands' own frame (§6.6e). (2) The **volume gauge is swept**, never cut across the off-scale 10-to-12 sector — the same fix. (3) The armed alarm **breathes red** instead of blinking. (4) Disarmed, the hands go to **both on the 6** rather than 12:00. (5) `clock` opens on the **clock's own time**, or 12:00 when it has never been set. (6) **One timeout, 5 s, every mode** — pairing's separate 120 s is gone | Setting a time is the one gesture where the hands follow *you* rather than a clock, and "take the shorter way" is the clock's rule, not yours; the hour hand moves twelve times slower and never hits the limit, which is why this read as "the minute hand is flaky". The rest is the same principle applied to reading: a blink says *going off* where a breath says *set*, 12:00 is a plausible time and was read as one where two hands stacked on the 6 is a reading no clock can produce, an unset chrono reads as minutes-since-boot, and a control with no labels can afford exactly one rule about how long it waits for you. Covered by `ux/tests/12-modes` + `13-wind` and by the host cases in `test_motion.cpp` |
...
| 2026-08-08 | **Protector suffix `HY2111-GB` → `HY2111-HB`** (discharge-OC `V_DIP` 150 mV → **200 mV**; every other threshold identical) | The -GB trips at **1.89 A worst case** across the AOSD32334C pair, and a loud alarm *while plugged* pulls **~2.3 A out of the cell** — `R18` limits the LT3652's contribution to ~1 A, so the cell covers the rest (`kicad/REVIEW.md` #6/#7). -HB moves the trip to **2.65 A**. Same SOT-23-6, same pinout, same R20/R21/C109 support network: **a BOM line, no schematic or PCB change** (values synced through `sync_pcb.py`). Verified against `battery_protector_hy2111.pdf` p.190–201 |
| 2026-08-08 | **Speaker stays `DMA58-4` (4 Ω); §16's rationale corrected** | The old note claimed 8 Ω would halve output — it would not, *when plugged*: at 4 Ω the ceiling is **L5/L6 saturation** (limiter caps 8 W at 79 % of Isat), at 8 Ω it is the 11.5 V rail (8.26 W). Plugged, both give ~8 W. The 4 Ω earns its place on **battery** only, where the rail clips: **3.1 W vs 1.6 W, +3 dB** |
