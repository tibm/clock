...
**One aluminum knob, top-center** (HiFi feel): smooth **no-detent** rotary encoder with an integral **push switch** — Bourns **PEC11R-4015F-S0024** (24 PPR quadrature ×4 on PCNT = 96 counts/rev; two channels A/B are fully sufficient, incl. direction; sensitivity is a firmware counts-per-step mapping). Off-board on **J10** (JST SH 1×05: GND · A · B · SW · GND); A/B → **IO47/48 (PCNT, glitch-filtered)**, press → **IO17** (host GPIO interrupt, RC + ~5 ms firmware debounce — deliberately *not* on the I²C expander).
...
| Knob encoder (smooth, push) | Bourns PEC11R-4015F-S0024 (no detent, 24 PPR) | $2.68 | [DigiKey 4499668 ✅](https://www.digikey.com/en/products/detail/bourns-inc/PEC11R-4015F-S0024/4499668) |
...
> **Status key:** ⭐ **selected** — datasheet on file in [`/datasheet`](datasheet/) · ⚠️ **needed, not yet locked** — exact part/verification/datasheet still to do · ➕ **improvement** — optional upgrade, not required for core function. *(Every ⭐ row has a matching datasheet — **except the v0.19 additions** (SK6812/2758, SN74AHCT1G125, PEC11R, USB4145, SH connectors), whose datasheets are still to be filed; `/datasheet` was deliberately left untouched this pass, so the dropped display/FPC datasheets also remain on file for future reuse.)*
...
| ⭐ | Knob encoder (top face, off-board) | **Bourns PEC11R-4015F-S0024** — **no detent** (smooth), 24 PPR, integral push switch, 15 mm flatted shaft | vertical TH, Ø12 bushing | $2.68 | [DigiKey ✅](https://www.digikey.com/en/products/detail/bourns-inc/PEC11R-4015F-S0024/4499668) |
...
| ⭐ | USB-C recept (**vertical**, power+CC+D±) | **GCT USB4145-03-0170-C** (USB 2.0 Type-C, 16-pin, stands on the PCB → port exits the back wall; 1.70 mm stakes for 1.6 mm board). *Replaces the horizontal USB4105 + Adafruit 6069 panel extension. The USB4140 the sketch referenced is the 6-pin power-only sibling — no D± → no USB-JTAG — so the 16-pin USB4145 is used.* | SMT vertical + 4 TH shell stakes | ~$1.05 | [DigiKey ✅](https://www.digikey.com/en/products/detail/gct/USB4145-03-0170-C/16649098) |
| ⭐ | Knob connector (J10) | **JST SM05B-SRSS-TB** (SH 1×05: GND·A·B·SW·GND) + pre-crimped SH cable | SH 1.0 mm, SMT R/A | ~$0.6 | [DigiKey ✅](https://www.digikey.com/en/products/detail/jst-sales-america-inc/SM05B-SRSS-TB/926711) |
| ⭐ | Radio-toggle connector (J11) | **JST SM02B-SRSS-TB** (SH 1×02: SIG·GND) → rear toggle switch | SH 1.0 mm, SMT R/A | ~$0.5 | [DigiKey ✅](https://www.digikey.com/en/products/detail/jst-sales-america-inc/SM02B-SRSS-TB/926708) |
...
**Core electronics subtotal (excl. speaker/cell/PCB): ~$95–115** — the v0.19 redesign removes the two priciest display lines (LS032B7DD02 ~$38 + FPC + panel extension ≈ **−$44**) and adds only ~$10 of UI parts (NeoPixel 10-pack + buffer + encoder + SH connectors); power electronics ≈ +$20–22 (LT3652 is the priciest line); the 2a sensor chain is 3 Adafruit **breakout modules** — BME688 + TSL2591 + LIS3DH ≈ **$31**. With speaker + user-supplied 18650 + holder + 4-layer PCB + passives ≈ **~$165–200**. +CO₂/PM ≈ +$62. *(Cell is user-supplied; safety HW is non-negotiable — see [`power.md`](power.md).)*

**Cost/space levers:** budget movement (VID28-05, −$10, off-DigiKey) + optical homing; skip CO₂/PM; the movement is now the big electronics line item — the enclosure (walnut + machined aluminum plate + knob + glass) dominates overall cost instead. *(Amp DSP is free — it runs in firmware.)*
...
- **Knob encoder:** Bourns **PEC11R-4015F-S0024** ⭐ (no detent, 24 PPR, push) / PEC11R-4215F (24-detent, if clicky is preferred) / Alps EC11 (detented) / magnetic (AS5600, overkill).
...
- **USB-C receptacle:** **GCT USB4145-03-0170-C** ⭐ (vertical, 16-pin, D±+CC) / USB4140 (vertical **power-only** — rejected, no D±) / USB4105-GF-A + Adafruit 6069 extension (the superseded horizontal + panel-remote combo).
...
| 2026-07-19 | **UI → single top knob: Bourns PEC11R-4015F-S0024** (no-detent, 24 PPR, push) on **J10** (SH 1×05); **ENC_SW → host IO17** (was expander GPA6); **BTN1–3 dropped**; **radio-disable toggle on J11** (SH 1×02, expander GPA3) | Press cycles the 5 status-LED modes, rotate edits (§12). No-detent + heavy aluminum knob = HiFi feel; 2-ch quadrature ×4 on PCNT (96 counts/rev) is fully sufficient. Press needs a crisp host interrupt + ~5 ms debounce — I²C-polled expander adds latency → moved to IO17 (freed by the display). Radio toggle is slow/static → expander |
| 2026-07-19 | **USB-C receptacle → vertical GCT USB4145-03-0170-C** (16-pin, USB 2.0); USB4105 + Adafruit 6069 extension dropped | Port now stands on the PCB and exits the back wall directly — no extension cable. ⚠ The sketch's **USB4140** is the **6-pin power-only** vertical variant (no D±) — it would kill native USB-JTAG flash/console, so its 16-pin sibling **USB4145** (D± + CC, 5 A VBUS, stake option 0170 for 1.6 mm boards) is used ([DigiKey 16649098](https://www.digikey.com/en/products/detail/gct/USB4145-03-0170-C/16649098), $1.05) |
