...
**Status:** v0.17 draft · **Owner:** you (FW/HW) · **Last updated:** 2026-07-12
...
- **A) Circular analog clock** — ~75 mm dial, two hands, driven by a dual-shaft concentric stepper with single-sensor **optical homing** (§5). Dial is **numeral-free / 4-fold-symmetric** (no printed numbers).
...
**How the MCU knows & adapts (R14):** the **LIS3DH accelerometer** reads the gravity vector → detects *flat vs standing* and *which 90°*. The MCU then:
1. **Rotates the display framebuffer** 90° so text stays upright.
2. **Re-references the analog "12"** — because the dial has no fixed numerals, the MCU drives both hands (absolute addressing + optical homing) to the correct positions for the new "up".
...
A **single MCU** must drive the SPI memory LCD, two stepper shafts, I²S audio, PWM LED lighting, the I²C sensor bus, and Wi-Fi + BLE. **No parallel-RGB / LTDC is needed** (the display is a low-bandwidth 3-wire SPI panel), which widens the field — but the **ESP32-S3 single-chip** still wins on simplicity + library support.

The N16R8 module exposes ~33 usable GPIO (octal PSRAM claims 3), and the pin budget is tight, so a **Microchip MCP23017** I²C IO expander (SOIC/SSOP-28, on the shared bus + one INT line) offloads the slow/static lines — amp mute, 12 V-boost enable, stepper STBY, rear buttons, encoder switch, and PD/charger status — for a **net −7 host GPIO**. The full pin-level allocation (host + expander) lives in [`esp32.md`](esp32.md) (**32/33 pads used**, native USB-JTAG on IO19/20, software VCOM); see also [`datasheet/README.md`](datasheet/README.md) §19.
...
| Orientation + tap (R14, R3) | **LIS3DH** driver (Adafruit_LIS3DH / esp-idf-lib): gravity-vector orientation (flat/standing) + hardware tap IRQ |
| Wi-Fi | `esp_wifi` |
...
**Fastest bring-up:** **ESPHome** gets sensors / SNTP / PWM LED (monochromatic) / I²S audio running in an afternoon; the memory LCD + steppers likely need a custom component or a drop to ESP-IDF. **esp-bsp** has ready board+display+LVGL configs.
...
| **Motion / tap / orientation** ⭐ | **ST LIS3DH** | tap engine **+ gravity-vector orientation (flat/standing, R14)**; low-res is fine (static gravity + tap IRQ). No leaded accel exists → rides a module on both build paths | $2 bare / ~$5 board |
| Hand homing ⭐ | **onsemi QRE1113** (reflective optical) | 1 sensor, sequential homing, **no hand magnets** (§5); analog out → 1 ADC; ≈3.6×2.9×1.7 mm (replaces the unavailable ITR8307) | ~$0.76 |
...
A single 3-axis accel covers **tap-to-snooze *and* orientation** — no gyro needed, since flat-vs-standing is a static gravity reading. All I²C on a shared bus (§14). **Vent the BME688 to outside air, away from amp/LEDs/battery** — their heat skews T/RH/VOC.
...
4-layer min (6 if dense); solid ground plane, RF keep-out (or use a module w/ onboard antenna to inherit certs). **3-wire SPI** to the memory LCD is short and low-bandwidth — **no length-matching needed** (the RGB pixel-clock group is gone with the bar TFT). **Two stepper shafts** via 2× TB6612FNG — keep coil traces + PWM away from RF/audio; mount **one reflective optical home sensor (QRE1113)** behind the dial, aligned to a punched index hole (shared by both hands via sequential homing). Class-D + speaker return away from RF/analog. Copper for charger/amp heat. FPC/ZIF for the display, JST for battery/speaker/LED/movement, SD cage, USB-C w/ ESD+CC. **Hand-solder rules:** leaded ICs only (no QFN/DFN/BGA/LGA); **≥0603 passives**; the PowerPAD amp + charger get a **thermal-via array** under the belly pad; leadless-only sensors ride on I²C **breakout modules**; use generous footprints/spacing — board area is traded for hand-assembly.
...
| Hand homing (×1, optical) | onsemi QRE1113 (reflective, analog out) | ~$0.76 | [DigiKey 2175990 ✅](https://www.digikey.com/en/products/detail/onsemi/QRE1113/2175990) |
...
> **Status key:** ⭐ **selected** — datasheet on file in [`/datasheet`](datasheet/) · ⚠️ **needed, not yet locked** — exact part/verification/datasheet still to do · ➕ **improvement** — optional upgrade, not required for core function. *(Every ⭐ row has a matching datasheet; the reverse also holds.)*
...
| ⭐ | IO expander (I²C, 16-bit) | **Microchip MCP23017** (SPI twin MCP23S17) — offloads SPK_SD / 12 V-EN / stepper STBY / buttons / enc-SW / PD-PG / CHRG / FAULT; INT→1 GPIO (net −7) | **SOIC-28 W / SSOP-28** | ~$1.7 | [DigiKey ✅](https://www.digikey.com/en/products/detail/microchip-technology/MCP23017T-E-SO/964184) |
| ⭐ | Info display | Sharp LS032B7DD02 | 3.16″, 47×76 mm | ~$38.2 | [DigiKey ✅](https://www.digikey.com/en/products/detail/sharp-microelectronics/LS032B7DD02/23349498) |
| ⭐ | Display FPC connector | Hirose FH34SRJ-10S-0.5SH(50) | **FPC/ZIF, 10-pos, 0.5 mm** | ~$1.1 | [DigiKey ✅](https://www.digikey.com/en/products/detail/hirose-electric-co-ltd/FH34SRJ-10S-0-5SH-50/3880272) |
| ⭐ | Sensor-board connector (off-board I²C + power: VCC/GND/SCL/SDA + 2 rsvd) | **JST B6B-ZR(LF)(SN)** header ×2 (one per board, TH vertical, shrouded/keyed, 1 A) + **A06ZR06ZR28H102B** pre-crimped cable ×1 (ZH, 6-pos, socket↔socket, 28 AWG, 102 mm) | **TH, 6-pos, 1.5 mm** | ~$0.24 ×2 + ~$1.54 | [DigiKey ✅](https://www.digikey.com/en/products/detail/jst-sales-america-inc/B6B-ZR(LF)(SN)/455-1661-ND/926568) [DigiKey ✅](https://www.digikey.com/en/products/result?keywords=A06ZR06ZR28H102B) |
| ⭐ | Analog movement | Juken X40.879 (dual shaft) | vertical, compact | ~$20 | [MiniTools ✅](https://store.minitools.com/en/sei-x40-879-juken-x40-879-stepper-motor.html) |
...
| ⭐ | Cell protector (independent) | AP9101CK6-BXTRG1 (OV 4.28 V) + AOSD32334C dual-N FET | **SOT-23-6 + SO-8** | ~$0.3 + ~$0.8 | [DigiKey ✅](https://www.digikey.com/en/products/detail/diodes-incorporated/AP9101CK6-BXTRG1/7352542) [DigiKey ✅](https://www.digikey.com/en/products/detail/alpha-omega-semiconductor-inc/AOSD32334C/11567511) |
...
| ⭐ | Transient | **Littelfuse SMAJ22A** (VBUS) + **SMAJ5.0A** (BAT) — 400 W uni | **DO-214AC (SMA)** | ~$0.5 x 2 | [DigiKey ✅](https://www.digikey.com/en/products/detail/littelfuse-inc/SMAJ22A/762286) [DigiKey ✅](https://www.digikey.com/en/products/detail/littelfuse-inc/SMAJ5-0A/762250) |
| ⭐ | Cell one-shot TCO (~77 °C) | thermal fuse in cell − path (SEFUSE SF / Bourns bimetal ~77 °C) — **pick exact part + file datasheet** | radial / tab | ~$0.9 | [DigiKey ✅](https://www.digikey.com/en/products/detail/cantherm/SDF-DF077S/1014754) |
...
| ⭐ | LED low-side switches ×3 | **AO3400A** N-FET (+ 100 Ω gate, 10 kΩ pulldown) — panel + 2× wake | **SOT-23** | ~$0.5 x 3 | [DigiKey ✅](https://www.digikey.com/en/products/detail/alpha-omega-semiconductor-inc/AO3400A/1855772) |
| ⭐ | 18650 holder | **Keystone 1043** (TH PC-pin, UL94V-0) — ventilated placement per `power.md` | TH leaf-spring | ~$3 | [DigiKey ✅](https://www.digikey.com/en/products/detail/keystone-electronics/1043/2745669) |
...
| ⭐ | Panel LEDs (5 V, ≤12) | Cree **CLM3C-MKW-CWAXB233** (warm 3200K, Vf 3.2 V, $0.31 ea) + ~180 Ω each | PLCC-2 | ~$0.31 x 10 | [DigiKey ✅](https://www.digikey.com/en/products/detail/cree-led-inc/CLM3C-MKW-CWAXB233/1987465) |
...
| ⭐ | USB-C recept (power+CC) | **GCT USB4105-GF-A** (USB 2.0 Type-C, 5 A) | SMT + TH tabs | ~$0.8 | [DigiKey ✅](https://www.digikey.com/en/products/detail/gct/USB4105-GF-A/11198441) |
...
**Core electronics subtotal (excl. speaker/cell/PCB): ~$134–159** (power electronics ≈ +$20–22 — the LT3652 is the priciest swap; the 2a sensor chain is 3 Adafruit **breakout modules** — BME688 + TSL2591 + LIS3DH ≈ **$31** (≈ +$18–20 vs bare parts; 2b daughterboard recovers most of that)). With speaker + user-supplied 18650 + holder + 4-layer PCB + passives ≈ **~$204–244**. +CO₂/PM ≈ +$62. *(Cell is user-supplied; safety HW is non-negotiable — see [`power.md`](power.md).)*

**Cost/space levers:** budget movement (VID28-05, −$10, off-DigiKey) + optical homing; skip CO₂/PM; the display + movement are the two big line items. *(Amp DSP is free — it runs in firmware.)*
...
- **Power:** **CH224K** (PD sink) + **LT3652** (1-cell buck charger, BAT-node path) + **ESP32 ADC** gauge + **AP9101C + AOSD32334C** protector + reverse P-FET; 1S Li-ion 18650 holder. *(All leaded/hand-solderable; CH224K off-DigiKey.)*
