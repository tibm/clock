...
| ⚠️ | 12 V-boost inductor (L4) | **Coilcraft XAL5030-472MEC** — 4.7 µH, **Isat 6.7 A**, Irms 5.9 A. The 4×4 mm `XAL4030-472` the footprint holds is only **Isat 4.6 A** < the ~6 A the 1S→12 V boost peaks at (`power_values.md` §5) → step to the **5×5 mm XAL5030** and change L4's land from `XAL4030` to `XAL5030` | 5.5×5.3×3.1 mm | ~$1.2 | [DigiKey](https://www.digikey.com/en/products/result?keywords=XAL5030-472MEC) |
| ⭐ | Speaker LC-filter inductor ×2 (L5, L6) | **Coilcraft XAL4040-103MEC** — 10 µH; ⚠️ verify Irms 2.2 A vs peak speaker current at max volume | 4.0×4.0×4.0 mm | ~$1.0 ×2 | [DigiKey](https://www.digikey.com/en/products/result?keywords=XAL4040-103MEC) |
...
| ⚠️ | Charger catch + 12 V-boost rectifier (D11, D20) | **B340A** (Schottky, 40 V / 3 A) — datasheet [Diodes DS30891](https://www.diodes.com/assets/Datasheets/ds30891.pdf) *(add to `/datasheet` — diodes.com blocks scripted fetch)* | SMA / DO-214AC | ~$0.30 | [DigiKey](https://www.digikey.com/en/products/result?keywords=B340A-13-F) |
| ⚠️ | Amp-mux 12 V blocking diode (D30) | **B340A** — schematic reads `SS34` on an **SMA** land, but SS34 is an **SMC (DO-214AB)** part; use **B340A** (SMA, matches the footprint + already in this BOM) or widen D30's land to SMC | SMA / DO-214AC | ~$0.30 | [DigiKey](https://www.digikey.com/en/products/result?keywords=B340A-13-F) |

*(XAL4020/4030/4040 share one 4×4 mm land, so the KiCad `L_Coilcraft_XAL4030-XXX` / `XAL4020-XXX` footprints already fit L1/L2/L3/L5/L6 — only L4's move to the 5×5 mm XAL5030 needs a new land + the XAL50xx datasheet. TVS D1/D12 → [`tvs_smaj.pdf`](datasheet/tvs_smaj.pdf); VBAT-sense clamp D13 BAT42W → [`diode_clamp_bat42w.pdf`](datasheet/diode_clamp_bat42w.pdf).)*
