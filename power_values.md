...
| **C_VIN** | **2×10 µF / 25 V** X7R low-ESR | RMS ripple = I_CHG/2 = 0.5 A worst case; TVS SMAJ22CA already on VBUS |
...
| **Vbat ADC divider** (+ disconnect FET, Decision #3) | R_top **100 k**, R_bot **100 k** (÷2: 4.2 V→2.1 V, ADC1 11 dB atten) + **100 nF** at node. **2N7002** in series with R_bot→GND, gate = `VBAT_DIV_EN` (expander) → off in deep-sleep (saves ~21 µA). **D13 BAT42W (SOD-123)** clamp, cathode on the node → bounds a reversed-cell excursion to ~−0.2 V (added 2026-07-21) |
| **CELL_TEST** (full-cell vs no-cell, added 2026-07-21) | Expander GPB7 → **Q8 2N7002** (gate PD **R26 100 k**) pulls **Q9 AO3401A**'s gate low (G-S pull-up **R27 100 k**); Q9 source = holder+, drain = Q2's gate → asserting lifts Q2's gate to holder+ → **Q2 off**. ADC step at switch-off ≥ ~0.3 V ⇒ empty holder (body-diode back-feed); ≤ ~50 mV ⇒ real cell. Plugged-only, brief; POR-safe (both FETs default off) |
| **USB D± ESD** (added 2026-07-21) | **USBLC6-2SC6** flow-through at J1: D− → pin 1 / out pin 6 → IO19, D+ → pin 3 / out pin 4 → IO20, GND pin 2. **VBUS pin 5 = NC on purpose** — our VBUS is the 15 V contract vs the pin's 5.25 V rating; the internal zener still clamps, and a cable VBUS-to-D± fault burns the (sacrificial) array instead of pumping a rail |
