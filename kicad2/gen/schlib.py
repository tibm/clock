...
class SymbolCache:
    ...
    def body_bbox(self, lib_id):
        """Graphics-only extents (rect/polyline/circle/arc, no pins/text) in
        library coords (Y up). Returns ((x0,y0),(x1,y1)) or None."""
        nick, name = lib_id.split(":", 1)
        flat = self._standalone_node(nick, name)
        xs, ys = [], []
        ...
        for sub in flat.findall("symbol"):
            for r in sub.findall("rectangle"):
                s, e = r.find("start"), r.find("end")
                take(s[1], s[2])
                take(e[1], e[2])
            for pl in sub.findall("polyline"):
                pts = pl.find("pts")
                if pts:
                    for xy in pts.findall("xy"):
                        take(xy[1], xy[2])
            for ci in sub.findall("circle"):
                cn, rad = ci.find("center"), ci.find("radius")
                cx, cy, r = float(cn[1]), float(cn[2]), float(rad[1])
                take(cx - r, cy - r)
                take(cx + r, cy + r)
            for ar in sub.findall("arc"):
