...
enum class Mod : uint8_t {
    ...
    drv_imu, drv_als, drv_env, drv_sd, drv_chg,
    count
};
...
[[nodiscard]] inline bool on(Mod m, Level l) noexcept {
    return static_cast<uint8_t>(l) <= g_level[static_cast<std::size_t>(m)].load(std::memory_order_relaxed);
}
    ...
    do {                                                                                   \
        if constexpr (::clk::log::Level::lvl_ <= ::clk::log::kCeiling) {                   \
            if (::clk::log::on(::clk::log::Mod::mod_, ::clk::log::Level::lvl_)) {           \
                ::clk::log::write(::clk::log::Mod::mod_,                                    \
                                  ::clk::log::Level::lvl_, __VA_ARGS__);                    \
            }                                                                              \
        }                                                                                  \
    } while (0)
