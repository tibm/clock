...
constexpr uint16_t kAll =
    bit(Dev::Opto) | bit(Dev::Vbat) | bit(Dev::Knob) | bit(Dev::Pixels) | bit(Dev::WakeLed) |
    bit(Dev::Expander) | bit(Dev::Amp) | bit(Dev::Als) | bit(Dev::Env) | bit(Dev::Imu) |
    bit(Dev::Sd) | bit(Dev::Xtal32k);
...
bool parse_dev(const char* s, Dev& out) noexcept {
    ...
    for (int i = 0; i < kDevCount; ++i) {
        if (std::strcmp(s, kDevNames[i]) == 0) { out = static_cast<Dev>(i); return true; }
    }
    return false;
}
...
void set_present(Dev d, bool on) noexcept {
    if (on) g_present |= bit(d);
    else    g_present = static_cast<uint16_t>(g_present & ~bit(d));
}
