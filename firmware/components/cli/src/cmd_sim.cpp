...
Status cmd_opto(Args const& a, Sink& out) {
    if (a.count() == 0) { out.printf("opto %.3f", sim::opto()); return Status::Ok; }
    const double v = arg_d(a, 0, -1);
    if (v < 0.0 || v > 1.0) { out.line("usage: sim opto <0..1>   (0 = dark, 1 = full reflection)"); return Status::BadArg; }
...
Status cmd_vbat(Args const& a, Sink& out) {
    ...
    if (mv < 2500 || mv > 4400) { out.line("usage: sim vbat <mV>   (2500-4400)"); return Status::BadArg; }
    sim::set_vbat_mv(static_cast<uint16_t>(mv));
    const auto p = hal::power::read();
    if (p.ok()) out.printf("vbat %u mV  soc %u%%  chrg=%d", p.v.vbat_mv, p.v.soc_pct, p.v.charging ? 1 : 0);
    return Status::Ok;
}

Status cmd_noise(Args const& a, Sink& out) {
    const long mv = arg_l(a, 0, -1);
    if (mv < 0 || mv > 500) { out.line("usage: sim noise <mV peak>   (0-500, 0 = off)"); return Status::BadArg; }
...
Status cmd_turn(Args const& a, Sink& out) {
    ...
    if (d == 0) { out.line("usage: sim turn <+/-detents>   (4 PCNT counts each)"); return Status::BadArg; }
    sim::turn(static_cast<int32_t>(d));
    const auto k = hal::knob::read();
    if (k.ok()) out.printf("knob count=%" PRId32 " (%+ld detents)", k.v.count, d);
...
Status cmd_press(Args const& a, Sink& out) {
    ...
    if (w && std::strcmp(w, "hold") == 0)         ms = kHoldForever;
    else if (w && std::strcmp(w, "release") == 0) ms = 0;
    else if (w)                                   ms = static_cast<uint32_t>(std::strtoul(w, nullptr, 10));
    sim::press(ms);
    if (ms == 0)                 out.line("ENC_SW released");
    else if (ms == kHoldForever) out.line("ENC_SW held (until `sim press release`)");
    else                         out.printf("ENC_SW pressed for %" PRIu32 " ms of sim time", ms);
    return Status::Ok;
}
...
Status cmd_warp(Args const& a, Sink& out) {
    if (a.count() == 0) { out.printf("warp %.2fx", sim::warp()); return Status::Ok; }
    const double x = arg_d(a, 0, -1);
    if (x <= 0) { out.line("usage: sim warp <factor>   (1 = real time, 60 = a minute per second)"); return Status::BadArg; }
    sim::set_warp(x);
...
Status cmd_jump(Args const& a, Sink& out) {
    ...
    if (s <= 0) { out.line("usage: sim jump <seconds>   (instant, does not run anything)"); return Status::BadArg; }
...
Status cmd_present(Args const& a, Sink& out) {
    ...
    board::Dev d{};
    if (!board::parse_dev(a.arg(0), d)) { out.printf("no such device '%s'", a.arg(0)); return Status::BadArg; }
    const char* v = a.arg(1);
    if (!v) { out.printf("%s %s", board::name(d), board::present(d) ? "fitted" : "absent"); return Status::Ok; }
...
Status cmd_status(Args const&, Sink& out) {
    ...
    out.printf("opto   %.3f", sim::opto());
    if (p.ok()) out.printf("power  %u mV  soc %u%%  plugged=%d  chrg=%d",
                           p.v.vbat_mv, p.v.soc_pct, p.v.plugged ? 1 : 0, p.v.charging ? 1 : 0);
    const auto k = hal::knob::read();
