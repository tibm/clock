...
Status s_vbat(char* out, std::size_t cap) {
    ...
    if (!p.ok()) return p.st;
    std::snprintf(out, cap, "mv=%u soc=%u plugged=%d chrg=%d",
...
Status s_knob(char* out, std::size_t cap) {
    ...
    if (!k.ok()) return k.st;
    std::snprintf(out, cap, "count=%" PRId32 " delta=%" PRId32 " sw=%d",
                  k.v.count, k.v.delta, k.v.sw ? 1 : 0);
    return Status::Ok;
}
...
Status cmd_list(Args const&, Sink& out) {
    ...
    for (auto const& s : kSensors) {
        if (!board::present(s.dev)) {
            ...
            out.printf("  %-7s  %-9s -                              %s", s.name, cmd::name(st), s.what);
        }
    }
...
Status cmd_read(Args const& a, Sink& out) {
    ...
    if (!s) { out.printf("no such sensor '%s'  (`sensor list`)", a.obj ? a.obj : ""); return Status::BadArg; }
    if (!board::present(s->dev)) { out.printf("%s: not present", s->name); return Status::NotPresent; }
    if (!s->has_driver) { out.printf("%s: no driver yet -- see FIRMWARE.md §12.1", s->name); return Status::NotReady; }
    char buf[96];
    const Status st = s->sample(buf, sizeof buf);
    if (st != Status::Ok) { out.printf("%s: %s", s->name, cmd::name(st)); return st; }
...
constexpr CmdSpec kRows[] = {
    ...
    { "sensor", kAnyObject,  "stream", "[<hz>] [<s>] [--csv] [--keep-logs]",
                                           "bounded stream, <=200 Hz, <=120 s",
                                           static_cast<uint16_t>(ReleaseOk | Streaming), cmd_stream },
