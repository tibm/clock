...
bool send_state(int fd) {
    ...
    std::snprintf(buf, sizeof buf,
                  "{\"t\":\"state\",\"ms\":%" PRIu64
                  ",\"warp\":%.3f,"
                  "\"hands\":{\"h\":%.3f,\"m\":%.3f,\"hp\":%" PRId32 ",\"mp\":%" PRId32
                  ",\"hv\":%" PRId32 ",\"mv\":%" PRId32
                  ",\"moving\":%s,\"motor\":%s},"
                  ...
                  "\"clock\":{\"h\":%d,\"m\":%d,\"s\":%d,\"valid\":%s,\"follow\":%s,\"prov\":%s,"
                  "\"sync\":%s},"
                  "\"px\":[%s],\"refreshed\":%s,"
                  "\"wake\":{\"warm\":%u,\"cool\":%u},"
                  "\"spk\":{\"on\":%s,\"vol\":%u},"
                  ...
                  s.motor_on ? "true" : "false", mo.state_name, mo.phase,
                  mo.homed ? "true" : "false", mo.target_hour, mo.target_minute, mo.home_ms,
                  mo.faults, uo.mode_name, uo.alarm_armed ? "true" : "false", uo.alarm_hour,
                  uo.alarm_minute, uo.volume, uo.idle_in_ms, uo.net_locked ? "true" : "false",
                  ...
                  s.cool_pct, s.spk_active ? "true" : "false", s.vol_pct,
                  s.plugged ? "true" : "false", s.vbat_mv, s.soc_pct, s.charging ? "true" : "false",
                  s.knob_count, s.knob_sw ? "true" : "false", static_cast<double>(s.opto),
                  s.opto_auto ? "true" : "false", static_cast<double>(s.yaw_deg), s.taps,
                  s.radio_off ? "true" : "false");
    return send_line(fd, buf);
}
