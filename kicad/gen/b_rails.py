...
def build(s):
    ...
    s.gnd(C120, "2", drop=0)
    # inductor VBAT -> SW
    L2 = s.L("L2", 222.25, 232.41, "1uH", fp="L4020", rot=90)
    s.pw(L2, "1", ("x", 195.58), ("y", 243.84))
    s.pw(L2, "2", ("x", 236.22), ("py", U5, "5"), ("pin", U5, "5"))
    s.gnd(U5, "4")
    ...
    s.gnd(R41, "2", drop=0)
    s.pw(U5, "1", ("x", 208.28), ("y", 269.24), ("x", 254.00), ("y", 253.99),
         ("px", R40, "2"))
    ...
    s.gnd(C133, "2", drop=0)
    # SYNC -> GND (not used)
    s.pw(U7, "6", ("x", 219.71), ("dy", 1.27))
    s.power_at(219.71, 326.39, "GND")
    # FREQ: R47 -> GND (500 kHz)
    R47 = s.R("R47", 222.25, 335.28, "95.3k",
    ...
    s.pw(C135, "1", ("y", 345.44), ("px", R48, "1"))
    s.gnd(C135, "2", drop=3.81)
    # FB divider: +12V -> R45 -> FB node -> R46 -> GND
    R46 = s.R("R46", 261.62, 372.11, "10k")
    s.gnd(R46, "2", drop=0)
